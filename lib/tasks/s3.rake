...
#    rake s3:retrieve:db:restore         # repopulate the db from the latest S3 backup, or optionally specify a VERSION=this_archine.tar.gz
...
namespace :s3 do
  ...
  def conn
    @s3_configs ||= YAML::load(ERB.new(IO.read("#{RAILS_ROOT}/config/amazon_s3.yml")).result)
    @conn ||= S3::AWSAuthConnection.new(@s3_configs[RAILS_ENV]['access_key_id'], @s3_configs[RAILS_ENV]['secret_access_key'], @s3_configs[RAILS_ENV]['use_ssl'])
  end
